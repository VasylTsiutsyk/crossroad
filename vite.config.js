...
const assetPrefix =
  templateConfig.output && templateConfig.output.bundleToAssets
    ? 'assets/'
    : '';
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: [
          {
            manualChunks(id) {
              // Якщо вимкнено бандлінг і дозволено code split для JS
              if (
                !templateConfig.js.bundle.enable &&
                templateConfig.js.codesplit
              ) {
                // Всі сторонні бібліотеки йдуть у окремий vendor-файл
                if (id.includes('node_modules')) {
                  return 'vendor';
                }
                // Увесь кастомний код збираємо в спільний app-файл
                if (id.includes(`${path.sep}src${path.sep}js${path.sep}`)) {
                  return 'app';
                }
                // Решта (наприклад, додаткові entry) зберігають свої назви
              }
              // Якщо бандлінг увімкнений, все піде в app (див. entry/chunk names)
            ...
            assetFileNames: asset => {
              ...
              let extType =
                asset.names && asset.names.length > 0
                  ...
                  : '';
              getPath = normalizeAssetPath(getPath);
              if (/css/.test(extType)) {
                return templateConfig.js.bundle.enable
                  ? `${assetPrefix}css/app.min[extname]`
                  : `${assetPrefix}css/[name].min[extname]`;
              } else {
                if (/eot|otf|ttf|woff|woff2/.test(extType)) {
                  extType = normalizeAssetPath('assets/fonts');
                } else {
                  extType = getPath;
                }
                return `${assetPrefix}${extType}/[name][extname]`; //-[hash]
              }
            },
            entryFileNames(chunkInfo) {
              // Якщо bundle.enable -> все у один app.min.js
              if (templateConfig.js.bundle.enable) {
                return `${assetPrefix}js/app.min.js`;
              }
              // Якщо codesplit вимкнено -> використовуємо стандартні назви entry
              if (!templateConfig.js.codesplit) {
                return `${assetPrefix}js/[name].min.js`;
              }
              // Примусово кладемо наш код у app, вендор у vendor, решта зберігає ім'я
              if (chunkInfo.name === 'vendor')
                return `${assetPrefix}js/vendor.min.js`;
              if (chunkInfo.name === 'app')
                return `${assetPrefix}js/app.min.js`;
              return `${assetPrefix}js/${chunkInfo.name}.min.js`;
            },
