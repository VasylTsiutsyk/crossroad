...
const enableFontsPreload = templateConfig.fonts?.preload !== false;
...
const iconsPreviewDir = 'src/assets/svgicons/preview';
const iconsCssDir = path.dirname(iconsCSSFile);

// Відносний шлях від iconfont.css до папки з шрифтом
const cssFontsUrl = path
  .relative(iconsCssDir, iconsPreviewDir) // ../../assets/svgicons/preview
  .replace(/\\/g, '/'); // щоб на Windows були нормальні слеші
...
const fontHtmlCss = () => {
  ...
  font-display: swap;
    ...
    fs.writeFile(fontsHTMLFile, enableFontsPreload ? linksToFonts : '', cb);
