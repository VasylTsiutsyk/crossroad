    ...
    ? [
        {
          ...
          writeBundle: {
            order: 'post',
            handler: async ({ dir }) => {
              const baseJsDir = templateConfig.output?.bundleToAssets
                ? `${dir}/assets/js`
                : `${dir}/js`;
              const jsFiles = globSync(`${baseJsDir}/*.js`);
              // Створення копій
              if (jsFiles.length) {
                const devDir = `${baseJsDir}/dev`;
                !fs.existsSync(devDir) ? fs.mkdirSync(devDir, { recursive: true }) : null;
